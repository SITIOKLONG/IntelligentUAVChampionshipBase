...
bool ErrorStateKalmanFilter::Predict(Eigen::Vector3d imu_acc, Eigen::Vector3d imu_gyr, Eigen::Vector3d& pos, Eigen::Vector3d& vel, Eigen::Vector3d& angle_vel, Eigen::Quaterniond& q,  long long tc)
{
    ...
    if (phi_norm > 1e-8) {
        R_i0_i1 += std::sin(phi_norm) / phi_norm * skew_sym_m_phi +
            (1 - std::cos(phi_norm)) / (phi_norm * phi_norm) * skew_sym_m_phi * skew_sym_m_phi;
    } else {
...
bool ErrorStateKalmanFilter::correct(Eigen::Vector3d gps_pos, Eigen::Quaterniond gps_q)
{
    m_mtx.lock();
    if (gps_q.norm() < 1e-6) {
        gps_q = Eigen::Quaterniond::Identity();
    } else {
        gps_q.normalize();
    }
    ...
    const Eigen::Vector3d delta_theta = m_X.block<3, 1>(INDEX_STATE_ORI, 0);
    const double delta_theta_norm = delta_theta.norm();
    if (delta_theta_norm > 1e-12) {
        Eigen::AngleAxisd err_r(delta_theta_norm, delta_theta / delta_theta_norm);
        m_pose.block<3, 3>(0, 0) =  m_pose.block<3, 3>(0, 0) * err_r.toRotationMatrix().inverse();
    }
