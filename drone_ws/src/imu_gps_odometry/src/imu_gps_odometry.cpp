...
namespace
{
long long stampToNsec(const ros::Time& stamp)
{
    return (stamp.isZero() ? ros::Time::now() : stamp).toNSec();
}

void initFilterFromPose(const geometry_msgs::PoseStamped& msg, const char* source)
{
    Eigen::Quaterniond q(
    ...
    pose.block<3, 1>(0, 3) << msg.pose.position.x,
        msg.pose.position.y,
        msg.pose.position.z;

    g_eskf_ptr->Init(pose, Eigen::Vector3d::Zero(), stampToNsec(msg.header.stamp));
    g_eskf_ptr->m_isInitailed = true;
    ROS_WARN_STREAM("[imu_gps_odometry] initialized from " << source);
}
}  // namespace
...
int main(int argc, char** argv)
{
    ...
    g_eskf_ptr = new ErrorStateKalmanFilter(-9.81083, 0.1, 0.1, 0.1, 0.0003158085227, 0.001117221, 0.5, 0.25, 0.00143, 0.0386);
    ros::init(argc, argv, "imu_gps_odometry"); // 初始化ros 节点，命名为 basic
...
void init_pose_ned_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    {
        initFilterFromPose(*msg, "/airsim_node/initial_pose");
    }
}
...
void odom_local_ned_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    if(!g_eskf_ptr->m_isInitailed)
    {
        initFilterFromPose(*msg, "/airsim_node/drone_1/gps");
    }
...
void imu_cb(const sensor_msgs::Imu::ConstPtr& msg)
{
    ...
    if(g_eskf_ptr->m_isInitailed)
    {
        ...
        Eigen::Quaterniond q;
        const bool predicted = g_eskf_ptr->Predict(Eigen::Vector3d(msg->linear_acceleration.x, msg->linear_acceleration.y, msg->linear_acceleration.z), 
